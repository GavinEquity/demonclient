...
@Slf4j
public class CutterPlugin extends LoopedPlugin
{
	@Inject
	private CutterConfig config;

	@Inject
	private OverlayManager overlayManager;

	@Inject
	private CutterOverlay cutterOverlay;
	...
	{
		overlayManager.add(cutterOverlay);
	}
	...
	{
		super.stop();
		overlayManager.remove(cutterOverlay);
	}

	@Subscribe
	public void onConfigButtonPressed(ConfigButtonClicked event)
	{
		if (!event.getGroup().contains("demon-cutter") || !event.getKey().toLowerCase().contains("start"))
		{
			return;
		}
	...
	@Provides
	CutterConfig provideConfig(ConfigManager configManager)
	{
		return configManager.getConfig(CutterConfig.class);
	}
